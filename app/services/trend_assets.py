        ...
        with warnings.catch_warnings():
            ...
            with Image.open(io.BytesIO(raw)) as source:
                ...
                if preserve_original:
                    # For a server-verified test output or integrity-checked
                    # copy of an already validated asset. Keep the exact bytes,
                ...
                image.info.clear()
                output = io.BytesIO()
                # Keep camera JPEGs compact instead of expanding every photograph
                # to PNG. Dimensions/orientation are preserved; metadata is stripped.
                ...
                return encoded, content_type, extension, {"width": width, "height": height}
...
def _store(*, raw: bytes, owner_id: int, trend_id: str, version_id: str | None, kind: str, result_type: str, generation_id: str | None = None, provenance: dict | None = None, preserve_validated_image: bool = False) -> dict:
    ...
    elif preserve_validated_image and result_type == "image":
        # Internal immutable copy only: bind_asset_to_version verifies the
        # stored descriptor first. User uploads always go through sanitization.
        raw, content_type, extension, metadata = _validated_image(raw, preserve_original=True)
    else:
    ...
    # Warm small display images from bytes already in memory. Older assets
    # acquire the same cache lazily on their first authorized display request.
    try:
        from app.services.trend_media import warm_display
        if kind in {"fixed", "cover", "preview"}:
            warm_display(payload, raw, persist=True)
    except Exception as exc:
        LOG.warning("Display cache deferred asset=%s error=%s", asset_id, type(exc).__name__)
    LOG.info("Asset saved kind=%s input_bytes=%s stored_bytes=%s elapsed_ms=%s",
             kind, input_size, len(raw), round((time.perf_counter() - started) * 1000))
    return result
...
def bind_asset_to_version(asset_id: str, owner_id: int, *, trend_id: str, version_id: str) -> dict:
    ...
    raw = bytes(_client().storage.from_(bucket).download(key))
    verified = bool(row.get("sha256")) and row.get("size_bytes") is not None
    if verified and (len(raw) != row["size_bytes"] or hashlib.sha256(raw).hexdigest() != row["sha256"]):
        raise TrendAssetError("Stored reference integrity mismatch")
    return _store(raw=raw, owner_id=owner_id, trend_id=trend_id, version_id=version_id,
                  kind=row["kind"], result_type=row["result_type"],
                  preserve_validated_image=verified,
