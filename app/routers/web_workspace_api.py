...
def _workspace_pending_legal_payload(
    response: Response,
    *,
    account: Dict[str, Any],
    legal_status: Optional[Dict[str, Any]] = None,
) -> Dict[str, Any]:
    """Выдаёт только auth-сессию, достаточную для подтверждения документов.

    Обычные защищённые endpoint'ы всё равно используют get_current_workspace_user
    и вернут LEGAL_CONSENT_REQUIRED, пока согласия не подтверждены. Главное отличие
    V5: после успешной проверки email/Telegram/MAX/Google пользователь НЕ теряет
    авторизацию и может нажать одну кнопку «Подтвердить документы и продолжить».
    """
    token_user = account_to_workspace_user_payload(account)
    access_token = create_access_token(user=token_user)
    _set_workspace_session_cookie(response, access_token)
    ...
    return {
        ...
        "code": "LEGAL_CONSENT_REQUIRED",
        "message": "Подтвердите текущие версии документов, чтобы продолжить работу.",
        "access_token": access_token,
        "token_type": "Bearer",
        "expires_in": WORKSPACE_SESSION_TTL_SEC,
        "user": token_user,
...
def _workspace_auth_after_identity(
    response: Response,
    *,
    account: Dict[str, Any],
    payload: Any,
    ...
    """Завершает вход без logout-loop при отсутствующих согласиях.

    1. Если текущие версии уже приняты — обычный полный вход.
    2. Если пользователь прямо сейчас отметил ОБЕ галочки — атомарно сохраняем и
       выдаём полный вход.
    3. Иначе выдаём ограниченную auth-сессию и просим подтвердить документы.
       Эта сессия годится для /legal/accept-current и /me, но не для генераций.
    """
    account_id = int(account.get("id") or 0)
    try:
    ...
    if bool(legal_status.get("complete")):
        result = _workspace_auth_payload(response, account=account)
        result["legal_status"] = legal_status
        return result

    if bool(getattr(payload, "terms_accepted", False)) and bool(getattr(payload, "personal_data_accepted", False)):
    ...
    return _workspace_pending_legal_payload(response, account=account, legal_status=legal_status)
...
async def workspace_auth_telegram(payload: TelegramAuthPayload, request: Request, response: Response) -> Dict[str, Any]:
    ...
    return _workspace_auth_after_identity(
        response, account=account, payload=payload, source="telegram_auth", request=request
    )
...
async def workspace_auth_google(payload: GoogleAuthPayload, request: Request, response: Response, user: Optional[Dict[str, Any]] = Depends(get_optional_workspace_user)) -> Dict[str, Any]:
    ...
    if user is not None:
        current_account = ensure_workspace_account_from_claims(user)
    ...
    return _workspace_auth_after_identity(
        response, account=account, payload=payload, source="google_auth", request=request
    )
...
async def workspace_auth_max_start(
    ...
    if intent == "link":
        ...
        # V5: уже существующая (в том числе restricted legal-pending) сессия
        # не должна ломать запуск MAX-login. Полный доступ всё равно закрыт
        # get_current_workspace_user до подтверждения документов.
        current_account = ensure_workspace_account_from_claims(user)
        link_account_id = int(current_account.get("id") or 0)
...
async def workspace_auth_max_webapp(payload: MaxWebAppAuthPayload, request: Request, response: Response, user: Optional[Dict[str, Any]] = Depends(get_optional_workspace_user)) -> Dict[str, Any]:
    ...
    if user is not None:
        if state_source in {"site_link", "link"}:
            ...
            # Restricted legal-pending session is still a valid identity session.
            # Do not force a 428 here; final access remains gated until acceptance.
            current_account = ensure_workspace_account_from_claims(user)
        current_account_id = int(current_account.get("id") or 0)
        login_requires_legal = False
    ...
    if login_requires_legal and legal_terms_accepted and legal_personal_data_accepted:
    ...
    class _FinalMaxLegalPayload:
        terms_accepted = legal_terms_accepted
        personal_data_accepted = legal_personal_data_accepted
    return _workspace_auth_after_identity(
        response,
        account=account,
        payload=_FinalMaxLegalPayload(),
        source="max_auth",
        request=request,
        evidence=legal_evidence,
    )
...
async def workspace_auth_max_status(state: str, response: Response) -> Dict[str, Any]:
    ...
    if row_status not in {"confirmed", "legal_required"} or not row.get("account_id"):
    ...
    if not bool(legal_status.get("complete")):
        result = _workspace_pending_legal_payload(response, account=account, legal_status=legal_status)
        result["status"] = "legal_required"
        return result
...
async def workspace_auth_email_login(payload: EmailLoginPayload, request: Request, response: Response) -> Dict[str, Any]:
    ...
    return _workspace_auth_after_identity(
        response, account=account, payload=payload, source="email_login", request=request
    )
...
async def workspace_auth_password_reset_confirm(payload: ResetPasswordConfirmPayload, response: Response) -> Dict[str, Any]:
    ...
    if not bool(legal_status.get("complete")):
        ...
        result["message"] = "Пароль изменён. Подтвердите текущие версии документов и продолжайте без повторного входа."
        return result
...
@router.get("/me")
async def workspace_me(response: Response, user: Dict[str, Any] = Depends(get_authenticated_workspace_user)) -> Dict[str, Any]:
    # /me — recovery endpoint. Он обязан вернуть личность даже когда текущая
    # редакция документов ещё не подтверждена, иначе UI не может показать
    # пользователю две галочки и получается logout-loop. Остальные защищённые
    # endpoint'ы по-прежнему используют get_current_workspace_user.
    account = ensure_workspace_account_from_claims(user)
    try:
        legal_status = current_legal_acceptance_status(account_id=int(account.get("id") or 0))
    except LegalConsentError as exc:
