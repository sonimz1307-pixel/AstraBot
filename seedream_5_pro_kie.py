...
KIE_SEEDREAM_5_PRO_T2I_MODEL = (
    os.getenv("KIE_SEEDREAM_5_PRO_T2I_MODEL") or "seedream/5-pro-text-to-image"
).strip() or "seedream/5-pro-text-to-image"
KIE_SEEDREAM_5_PRO_I2I_MODEL = (
    os.getenv("KIE_SEEDREAM_5_PRO_I2I_MODEL") or "seedream/5-pro-image-to-image"
).strip() or "seedream/5-pro-image-to-image"
KIE_SEEDREAM_5_PRO_CALLBACK_URL = (os.getenv("KIE_SEEDREAM_5_PRO_CALLBACK_URL") or "").strip()
KIE_SEEDREAM_5_PRO_CREATE_TIMEOUT_SECONDS = float(
    os.getenv("KIE_SEEDREAM_5_PRO_CREATE_TIMEOUT_SECONDS", "60") or "60"
)
KIE_SEEDREAM_5_PRO_MAX_WAIT_SECONDS = float(
    os.getenv("KIE_SEEDREAM_5_PRO_MAX_WAIT_SECONDS", "900") or "900"
)
KIE_SEEDREAM_5_PRO_MAX_REFS = max(
    1, min(10, int(os.getenv("KIE_SEEDREAM_5_PRO_MAX_REFS", "10") or "10"))
)
KIE_SEEDREAM_5_PRO_MAX_INPUT_MB = max(
    1, min(10, int(os.getenv("KIE_SEEDREAM_5_PRO_MAX_INPUT_MB", "10") or "10"))
)
KIE_SEEDREAM_5_PRO_MAX_INPUT_BYTES = KIE_SEEDREAM_5_PRO_MAX_INPUT_MB * 1024 * 1024

_ALLOWED_QUALITIES = {"1K": "basic", "2K": "high"}
_ALLOWED_ASPECT_RATIOS = {"1:1", "4:3", "3:4", "16:9", "9:16", "2:3", "3:2"}
_ALLOWED_REFERENCE_EXTENSIONS = {"jpg", "jpeg", "png", "webp"}
_POLLING_STATES = {
    "waiting", "queuing", "queued", "generating", "processing", "pending", "submitted", "created", "running"
}
...
def validate_seedream_5_pro_reference_bytes(
    payload: bytes,
    *,
    filename: Any = None,
    ...
    if not isinstance(payload, (bytes, bytearray)) or not payload:
        raise Seedream5ProProviderError(f"{source_label}: пустой файл изображения")
    if len(payload) > KIE_SEEDREAM_5_PRO_MAX_INPUT_BYTES:
        raise Seedream5ProProviderError(
            f"{source_label}: файл больше {KIE_SEEDREAM_5_PRO_MAX_INPUT_MB} МБ"
        )

    ext = (
        _reference_ext_from_bytes(bytes(payload))
        or _reference_ext_from_name(filename)
        or _reference_ext_from_mime(content_type)
    )
    if ext == "jpeg":
        ext = "jpg"
    if ext not in _ALLOWED_REFERENCE_EXTENSIONS:
        raise Seedream5ProProviderError(
            f"{source_label}: неподдерживаемый формат. Для Seedream 5.0 Pro нужны JPG, PNG или WEBP; HEIC/HEIF не отправляем в KIE."
        )
    mime = {"jpg": "image/jpeg", "png": "image/png", "webp": "image/webp"}.get(ext, "image/jpeg")
    return ext, mime


def normalize_seedream_5_pro_resolution(value: Any, default: str = "2K") -> str:
    raw = str(value or default).strip().upper() or default
    return raw if raw in _ALLOWED_QUALITIES else default


def normalize_seedream_5_pro_aspect_ratio(value: Any, default: str = "16:9") -> str:
...
def normalize_seedream_5_pro_options(
    resolution: Any,
    aspect_ratio: Any,
    *,
    ...
    return (
        normalize_seedream_5_pro_resolution(resolution, default=default_resolution),
        normalize_seedream_5_pro_aspect_ratio(aspect_ratio, default=default_aspect),
    )


def seedream_5_pro_kie_cost(resolution: Any = "2K") -> int:
    # NABEX minimum billing unit is one token. Both KIE qualities stay within it.
    normalize_seedream_5_pro_resolution(resolution)
    return 1


def _auth_headers() -> Dict[str, str]:
    if not KIE_API_TOKEN:
        raise Seedream5ProProviderError("Seedream 5.0 Pro provider token is not configured.")
...
async def _kie_request_json(
    ...
    *,
    ...
    resp = await client.request(
        method.upper(), f"{KIE_API_BASE}{path}", headers=_auth_headers(), params=params, json=payload
    )
    text = resp.text or ""
    try:
    ...
    if resp.status_code >= 400:
        ...
        raise Seedream5ProProviderError(f"Seedream 5.0 Pro request failed ({resp.status_code}): {detail}")
    if isinstance(data, dict):
        ...
        if code not in {"0", "200"} and msg != "success":
            ...
            raise Seedream5ProProviderError(f"Seedream 5.0 Pro API error: {detail}")
...
async def _resolve_source_urls(
    *,
    ...
    def _add(raw: Any) -> None:
        ...
        if not value or value in seen or "/file/bot" in value:
            return
    ...
    if (telegram_file_id or telegram_file_ids) and not resolved:
        raise Seedream5ProProviderError(
            "Telegram images must be uploaded to public storage before calling Seedream 5.0 Pro provider."
        )
    return resolved[:KIE_SEEDREAM_5_PRO_MAX_REFS]
...
async def _poll_task(client: httpx.AsyncClient, task_id: str) -> str:
    ...
    while True:
        ...
        if state:
            last_state = state
        if state == "success":
            image_url = _extract_image_url(data.get("resultJson")) or _extract_image_url(data)
            if not image_url:
                raise Seedream5ProProviderError(
                    f"Seedream 5.0 Pro task succeeded but no image url was returned: {data}"
                )
            return image_url
        ...
        if (time.monotonic() - start_ts) >= KIE_SEEDREAM_5_PRO_MAX_WAIT_SECONDS:
            raise Seedream5ProProviderError(
                f"Seedream 5.0 Pro timeout. Last state: {last_state or 'unknown'} {last_detail}".strip()
            )
...
async def _download_bytes(url: str, *, timeout: float = 300.0) -> bytes:
    ...
    if not target:
        raise Seedream5ProProviderError("Empty image url")
...
async def handle_seedream_5_pro_kie(
    prompt: str,
    *,
    ...
    if not clean_prompt:
        raise Seedream5ProProviderError("Empty prompt")

    normalized_resolution, normalized_aspect = normalize_seedream_5_pro_options(resolution, aspect_ratio)
    ...
    if is_i2i and not source_urls:
        raise Seedream5ProProviderError("Seedream 5.0 Pro Image→Image requires at least one reference image")

    input_payload: Dict[str, Any] = {
        "prompt": clean_prompt,
        "aspect_ratio": normalized_aspect,
        "quality": _ALLOWED_QUALITIES[normalized_resolution],
        "output_format": "png",
        "nsfw_checker": True,
    }
    if is_i2i:
        input_payload["image_urls"] = source_urls[:KIE_SEEDREAM_5_PRO_MAX_REFS]

    payload: Dict[str, Any] = {
        "model": KIE_SEEDREAM_5_PRO_I2I_MODEL if is_i2i else KIE_SEEDREAM_5_PRO_T2I_MODEL,
        "input": input_payload,
    }
    if KIE_SEEDREAM_5_PRO_CALLBACK_URL:
        payload["callBackUrl"] = KIE_SEEDREAM_5_PRO_CALLBACK_URL
