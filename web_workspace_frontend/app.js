...
const DEFAULT_VIDEO = JSON.parse(localStorage.getItem('astrabot:videoState') || 'null') || {};
...
const state = {
  ...
  video: {
    provider: DEFAULT_VIDEO.provider || 'kling',
    model: DEFAULT_VIDEO.model || 'kling-3.0',
    mode: DEFAULT_VIDEO.mode || 'text_to_video',
    prompt: DEFAULT_VIDEO.prompt || '',
    duration: DEFAULT_VIDEO.duration || '5',
    resolution: DEFAULT_VIDEO.resolution || '720',
    aspectRatio: DEFAULT_VIDEO.aspectRatio || '16:9',
    enableAudio: !!DEFAULT_VIDEO.enableAudio,
    quality: DEFAULT_VIDEO.quality || 'pro',
    outputUrl: DEFAULT_VIDEO.outputUrl || '',
    providerTaskId: DEFAULT_VIDEO.providerTaskId || '',
    statusKind: DEFAULT_VIDEO.statusKind || 'idle',
    statusText: DEFAULT_VIDEO.statusText || 'После запуска здесь появятся ход генерации, ошибки и итоговое видео.',
    lastStatus: DEFAULT_VIDEO.lastStatus || '',
    errorText: DEFAULT_VIDEO.errorText || '',
  },
  image: {
...
function saveState() {
  ...
  localStorage.setItem('astrabot:videoState', JSON.stringify({
    ...
    statusKind: state.video.statusKind || 'idle',
...
async async function logoutWorkspace() {
  stopVideoPolling();
  state.authToken = '';
  state.me = null;
  state.balance = null;
  ...
  render();
  try { await apiFetch('/api/workspace/logout', { method: 'POST' }); } catch {}
  toast('success', 'Выход выполнен', 'Сессия сайта закрыта.');
}
...
function getVideoStatusMeta(kind, rawStatus = '') {
  switch (kind) {
    case 'queued':
      return { label: 'В очереди', badgeClass: 'warn', title: 'Генерация запущена' };
    case 'running':
      return { label: 'Обработка', badgeClass: 'ok', title: 'Генерация идёт' };
    case 'done':
      return { label: 'Готово', badgeClass: 'ok', title: 'Видео готово' };
    case 'error':
      return { label: 'Ошибка', badgeClass: 'error', title: 'Генерация завершилась с ошибкой' };
    default:
      return { label: rawStatus || 'Ожидание', badgeClass: 'muted', title: 'Video workspace' };
  }
}

function inferVideoStatusKind(status = '', hasOutput = false) {
  if (hasOutput) return 'done';
  const s = String(status || '').toLowerCase();
  if (!s) return 'running';
  if (['done', 'success', 'succeeded', 'completed', 'complete', 'finished'].some(k => s.includes(k))) return 'done';
  if (['fail', 'error', 'cancel', 'rejected', 'blocked'].some(k => s.includes(k))) return 'error';
  if (['queue', 'pending', 'submit', 'created', 'waiting'].some(k => s.includes(k))) return 'queued';
  return 'running';
}
...
function startVideoPolling(immediate = false) {
  stopVideoPolling();
  if (!state.video.providerTaskId || state.video.outputUrl || state.video.statusKind === 'error') return;
  const runner = () => pollVideoTask(false);
  if (immediate) setTimeout(runner, 1200);
  runtime.videoPollTimer = setInterval(runner, 8000);
}

function renderVideoRunCard() {
  const meta = getVideoStatusMeta(state.video.statusKind, state.video.lastStatus);
  const safePrompt = escapeHtml(state.video.prompt || '—');
  return `
    <div class="result-card" style="width:min(760px,100%); text-align:left;">
      <div class="field-head">
        <h4>${escapeHtml(meta.title)}</h4>
        <span class="badge ${meta.badgeClass}">${escapeHtml(meta.label)}</span>
      </div>
      <small>${escapeHtml(state.video.statusText || 'После запуска здесь появится ход генерации, ошибки и итоговое видео.')}</small>
      <div class="summary-grid two" style="margin-top:12px;">
        ...
        <div class="asset-card">
          <h4>Task ID</h4>
          <small style="word-break:break-all;">${escapeHtml(state.video.providerTaskId || '—')}</small>
        </div>
      </div>
      <div class="link-row" style="grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); margin-top:12px;">
        ${state.video.providerTaskId ? `<button class="btn ghost" data-action="poll-video-task">Проверить статус</button>` : ''}
        ${state.video.outputUrl ? `<a class="btn ghost" href="${escapeHtml(state.video.outputUrl)}" target="_blank" rel="noopener noreferrer">Открыть видео</a>` : ''}
        ${state.video.outputUrl ? `<a class="btn primary" href="${escapeHtml(state.video.outputUrl)}" download>Скачать видео</a>` : ''}
      </div>
      <small style="margin-top:12px;">Prompt: ${safePrompt}</small>
    </div>
  `;
}
...
function renderVideoWorkspace() {
  ...
  const stageBody = state.video.outputUrl ? `
    <div style="width:min(860px,100%); display:grid; gap:16px; align-items:start;">
      <video class="preview-media" src="${escapeHtml(state.video.outputUrl)}" controls playsinline></video>
      ${renderVideoRunCard()}
    </div>
  ` : `
    <div style="width:min(760px,100%); display:grid; gap:18px; justify-items:center;">
      ${renderVideoRunCard()}
      <div class="empty-copy">
        <strong>Video workspace</strong>
        <div>После запуска вся информация живёт здесь: старт генерации, ход обработки, ошибки и итоговое видео. Отдельный правый статус-блок убран, чтобы рабочая зона была одной и понятной.</div>
      </div>
    </div>
  `;
...
async async function runVideo() {
  ...
  if (provider === 'kling' && model === 'kling-3.0' && mode === 'text_to_video') {
    ...
    try {
      ...
      state.video.statusKind = 'queued';
      state.video.lastStatus = 'submitted';
      state.video.statusText = 'Генерация запущена. Задача отправлена в backend и будет отображаться прямо в рабочей зоне.';
      render();
      saveState();
      ...
      state.video.statusKind = 'queued';
      state.video.lastStatus = 'submitted';
      state.video.statusText = `Генерация запущена. Токенов списано: ${data.tokens_required || '—'}. Результат появится здесь же в центре.`;
      pushRun({ studio: 'Video', title: 'Kling 3 Text → Video', summary: state.video.prompt.slice(0, 100) });
      saveState();
      render();
      startVideoPolling(true);
      toast('success', 'Kling 3 запущен', 'Задача отправлена в backend. Дальше статус подтянется автоматически.');
    } catch (e) {
      state.video.statusKind = 'error';
      state.video.errorText = String(e.message || e);
      state.video.statusText = `Ошибка запуска: ${state.video.errorText}`;
      saveState();
      render();
      toast('error', 'Kling 3 create error', state.video.errorText);
    }
    return;
  }
...
async async function pollVideoTask(showToast = true) {
  if (!state.video.providerTaskId) {
    if (showToast) toast('error', 'Нет task id', 'Сначала создай задачу Kling 3.');
    return;
  }
  try {
    ...
    const status = task.status || task.state || (task.data || {}).status || (task.output || {}).status || 'unknown';
    const maybeUrl = task.output?.video_url || task.video_url || (task.data || {}).video_url || (task.output || {}).url || '';
    const errorText = task.error?.message || task.error_message || task.message || task.detail || '';

    state.video.lastStatus = status;
    if (maybeUrl) state.video.outputUrl = maybeUrl;
    state.video.statusKind = inferVideoStatusKind(status, !!maybeUrl);

    if (state.video.statusKind === 'done') {
      state.video.statusText = 'Видео готово и загружено в рабочую зону.';
      stopVideoPolling();
      pushRun({ studio: 'Video', title: 'Kling result ready', summary: state.video.prompt.slice(0, 100) });
      if (showToast) toast('success', 'Видео готово', 'Результат появился в рабочей зоне.');
    } else if (state.video.statusKind === 'error') {
      state.video.errorText = errorText || `Провайдер вернул статус: ${status}`;
      state.video.statusText = `Ошибка генерации: ${state.video.errorText}`;
      stopVideoPolling();
      pushRun({ studio: 'Video', title: 'Kling error', summary: state.video.errorText.slice(0, 100) });
      if (showToast) toast('error', 'Ошибка генерации', state.video.errorText);
    } else {
      state.video.statusText = `Генерация в работе. Текущий статус провайдера: ${status}.`;
      if (showToast) toast('success', 'Статус обновлён', `Kling task: ${status}`);
    }
    ...
    state.video.statusKind = 'error';
    state.video.errorText = String(e.message || e);
    state.video.statusText = `Ошибка проверки статуса: ${state.video.errorText}`;
    stopVideoPolling();
    saveState();
    render();
    if (showToast) toast('error', 'Не удалось проверить статус', state.video.errorText);
  }
}
...
function resetCurrentStudio() {
  switch (state.studio) {
    ...
    case 'video':
      ...
      state.video.outputUrl = '';
      state.video.statusKind = 'idle';
      state.video.lastStatus = '';
      state.video.errorText = '';
...
async function init() {
  ...
  if (state.video.providerTaskId && !state.video.outputUrl && state.authToken) startVideoPolling(true);
  render();
}
