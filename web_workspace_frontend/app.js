...
const DEFAULT_TELEGRAM_ID = localStorage.getItem('astrabot:telegramUserId') || '';
...
const state = {
  ...
  video: {
    ...
    outputUrl: '',
    providerTaskId: '',
    statusText: 'Здесь будут превью, статусы и готовые видео.',
  },
  image: {
...
function saveState() {
  ...
  localStorage.setItem('astrabot:telegramUserId', state.telegramUserId);
...
function requireTelegramId() {
  if (!String(state.telegramUserId || '').trim()) {
    toast('error', 'Нужен Telegram ID', 'Укажи Telegram User ID слева, чтобы использовать баланс и запускать боевые действия.');
    return false;
  }
  return true;
}
...
function renderHeader() {
  ...
  document.getElementById('telegramUserId').value = state.telegramUserId;
...
function renderVideoWorkspace() {
  ...
  const backendStatus = model?.backend === 'partial' ? 'Частично подключено' : (model?.backend === 'planned' ? 'Готово для интеграции' : 'Подключено');
  const uploadedCards = [
    ...
    <div class="workspace-grid">
        ...
        <div class="placeholder-stage video">
          ...
          ` : `
            <div class="empty-copy">
              ...
              <div>Выбери модель, режим и нужные входы справа. В этом центре будут: превью, итоговое видео, статусы генерации и рабочие ассеты.</div>
            </div>
          `}
        </div>
      ...
      <div class="workspace-side scroll">
        <div class="result-card">
          <div class="field-head"><h4>Статус модели</h4><span class="badge ${model?.backend === 'planned' ? 'warn' : 'ok'}">${backendStatus}</span></div>
          <small>${escapeHtml(state.video.statusText || '')}</small>
        </div>
        <div class="result-card">
          <h4>Текущий запуск</h4>
          <div class="tableish">
            <div class="table-row"><span class="muted">Task ID</span><span>${escapeHtml(state.video.providerTaskId || '—')}</span><button class="btn ghost small" data-action="poll-video-task">Проверить</button></div>
            <div class="table-row"><span class="muted">Prompt</span><span>${escapeHtml(state.video.prompt || '—')}</span><span class="badge muted">${escapeHtml(state.video.mode)}</span></div>
          </div>
        </div>
        <div class="result-card">
          <h4>Что уже live</h4>
          <small>Сейчас в боевом API доступен Kling 3 Text → Video. Другие видео-сценарии уже заложены в архитектуру интерфейса и готовы к backend-интеграции.</small>
        </div>
      </div>
    </div>
  `;
      ...
      <div class="workspace-main scroll">
        ...
        <div class="upload-grid two" style="margin-top:16px;">
          <div class="billing-card">
            ...
            <small>Wallet уже можно читать через `/api/workspace/balance/{telegram_user_id}`. В будущем сюда можно перенести пакеты, YooKassa / Stars, историю hold/refund и детализацию затрат по моделям.</small>
          </div>
          <div class="billing-card">
    ...
    <div class="workspace-grid">
      <div class="workspace-main scroll">
        <div class="profile-card">
          <h4>Профиль</h4>
          <div class="tableish">
            <div class="table-row"><span class="muted">Telegram User ID</span><span>${escapeHtml(String(state.telegramUserId || '—'))}</span><span class="badge muted">workspace</span></div>
...
function renderLibraryInspector() {
  return `
    <div class="inspector-card">
      ...
      <div class="help-text">Эта студия уже использует существующие роуты `/api/prompts/categories`, `/groups`, `/items` и не требует новой серверной логики для чтения.</div>
...
async function loadBalance() {
  ...
  try {
    const res = await apiFetch(`/api/workspace/balance/${encodeURIComponent(state.telegramUserId)}`);
    const data = await res.json();
...
async function loadPromptCategories() {
  ...
  render();
  try {
    const res = await apiFetch('/api/prompts/categories');
    const data = await res.json();
...
async function loadPromptGroups(category) {
  ...
  state.prompts.items = [];
  try {
    const res = await apiFetch(`/api/prompts/groups?category=${encodeURIComponent(category)}`);
    const data = await res.json();
...
async function loadPromptItems(groupId) {
  ...
  try {
    const res = await apiFetch(`/api/prompts/items?group_id=${encodeURIComponent(groupId)}`);
    const data = await res.json();
...
async function sendChat() {
  ...
  try {
    const res = await apiFetch('/api/workspace/chat', {
      ...
      body: JSON.stringify({
        telegram_user_id: state.telegramUserId ? Number(state.telegramUserId) : null,
        text: outgoing,
...
async function runVideo() {
  ...
  if (provider === 'kling' && model === 'kling-3.0' && mode === 'text_to_video') {
    ...
    try {
      const res = await apiFetch('/api/kling3/create', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          telegram_user_id: Number(state.telegramUserId),
      ...
      state.video.statusText = `Задача создана. Токенов списано: ${data.tokens_required || '—'}. Используй кнопку проверки статуса.`;
      pushRun({ studio: 'Video', title: 'Kling 3 Text → Video', summary: state.video.prompt.slice(0, 100) });
      toast('success', 'Kling 3 запущен', 'Задача отправлена в backend.');
      render();
    } catch (e) {
...
async function pollVideoTask() {
  if (!state.video.providerTaskId) {
    toast('error', 'Нет task id', 'Сначала создай задачу Kling 3.');
    return;
  }
  try {
    const res = await apiFetch(`/api/kling3/task/${encodeURIComponent(state.video.providerTaskId)}`);
    const data = await res.json();
    const task = data.task || {};
    const status = task.status || task.state || (task.data || {}).status || 'unknown';
    state.video.statusText = `Статус: ${status}`;
    ...
    pushRun({ studio: 'Video', title: 'Kling task poll', summary: `Status: ${status}` });
    toast('success', 'Статус обновлён', `Kling task: ${status}`);
    render();
  } catch (e) {
    toast('error', 'Не удалось проверить статус', String(e.message || e));
  }
}
...
async function runSongwriter() {
  ...
  try {
    const res = await apiFetch('/api/songwriter_lyrics', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
...
function handleInputChange(target) {
  ...
  switch (id) {
    case 'telegramUserId': state.telegramUserId = value; break;
...
document.addEventListener('click', (e) => {
  ...
  if (e.target.id === 'saveSettingsBtn') {
    state.telegramUserId = document.getElementById('telegramUserId').value.trim();
    state.apiBaseUrl = document.getElementById('apiBaseUrl').value.trim();
    saveState();
    renderHeader();
    toast('success', 'Настройки сохранены', 'API Base URL и Telegram User ID сохранены.');
    return;
  }
